...
export default function Carausal() {
  const duplicated = [...carausel, ...carausel];
  return (
    <div className="px-4 sm:px-6 lg:px-12 py-6 overflow-hidden">
      <Slider {...settings}>
        {duplicated.map((el) => (
          <Images key={`${el.id}-${Math.random()}`} el={el} />
        ))}
      </Slider>
    </div>
  );
...
function Images({ el }) {
  return (
    <div className="flex items-center justify-center mx-2 sm:mx-4">
      <Image
        className="w-full h-auto object-contain max-w-[100px] sm:max-w-[120px] lg:max-w-[150px]"
        width={500}
        height={500}
        src={el.img}
        alt="client image"
